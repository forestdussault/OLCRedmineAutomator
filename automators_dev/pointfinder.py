...
def pointfinder_redmine(redmine_instance, issue, work_dir, description):
    ...
    # Directory to store all the pointfinder outputs
    ...
    for seqid in sorted(seqids):
        # If the seqid isn't present in the dictionary, it is because the assembly could not be found
        try:
            ...
            if pointfinder_genus in pointfinder_list:
                ...
                output_folder = os.path.join(work_dir, seqid, 'output')
                try:
                ...
                outputs = glob.glob(os.path.join(output_folder, '*.txt'))
                for output in outputs:
                    shutil.move(output, pointfinder_output_dir)
            else:
